...
create table if not exists suspension_methods (
  ...
  catalog_name text not null default 'תיאור ציוד',
...
alter table suspension_methods add column if not exists catalog_name text not null default 'תיאור ציוד';
...
create index if not exists customer_equipment_row_params_customer_id_idx on customer_equipment_row_params(customer_id);
create index if not exists customer_equipment_row_params_equipment_catalog_id_idx on customer_equipment_row_params(equipment_catalog_id);
