...
with seed_rows (code, name, description_template, params_json, sort_order, is_default_equipment_row) as (
  values
    (
      'guardrail_clamps',
...
insert into suspension_methods
  (owner_id, code, name, description_template, params_json, sort_order, is_default_equipment_row)
select
  auth.users.id,
  seed_rows.code,
  seed_rows.name,
